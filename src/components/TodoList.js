...
const TodoList = ({ tasks }) => {
  ...
  const addNewTask = (e) => {
    e.preventDefault();
    const _id = uuidv4();
    dispatch(addTask({ ...defaultTask, title, id: _id, createdAt: new Date().getTime() }));
    setTitle('');
  }
  ...
  //     },
  //     body: JSON.stringify({ title, id: _id, createdAt: new Date().getTime() })
  //   })
  //     .then(resp => {
  //       dispatch(addTask({ ...defaultTask, title, id: _id, createdAt: new Date().getTime() }))
  //     })
      ...
      <div className="tasks-container">
        ...
        {sortedTasks.length > 0 &&
          <div className='tasks-container-header'>
            {sortedTasks.filter(task => !task.completed).filter(task => !categorisedTasks.today.includes(task)).filter(task => !categorisedTasks.thisWeek.includes(task))
              .filter(task => !categorisedTasks.overDue.includes(task)).map((task, index) => <TaskItem task={task} index={index} key={index} />)} </div>}
        {/* Empty Tasks */}
